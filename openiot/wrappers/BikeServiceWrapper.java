...
public class BikeServiceWrapper extends AbstractWrapper {
   
    private int DEFAULT_RATE = 2000;

    private static int threadCounter = 0;

    private final transient Logger logger = Logger.getLogger(BikeServiceWrapper.class);
    ...
    private transient final DataField[] outputStructure = new DataField[]{
        new DataField("stationid", "integer", "Station id"),
        new DataField("available", "integer", "If there are any bikes available")
    };
    ...
    */
    public boolean initialize() {
        ...
        if (logger.isDebugEnabled()) {
            logger.debug("BikeService is now running @" + rate + " Rate.");
        }
        return true;
    }
