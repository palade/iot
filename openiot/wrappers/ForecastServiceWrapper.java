...
public class ForecastServiceWrapper extends AbstractWrapper {
   
    private int DEFAULT_RATE = 2000;

    private static int threadCounter = 0;

    private final transient Logger logger = Logger.getLogger(ForecastServiceWrapper.class);
   ...
   private transient final DataField [] outputStructure = new  DataField [] { 
       new DataField( "rain" , "integer" , "Rain status from weather station" ),
       new DataField( "temperature" , "double" , "Temperature status from weather station" )
   };
    ...
    */
    public boolean initialize() {
        ...
        if (logger.isDebugEnabled()) {
            logger.debug("ForecastService is now running @" + rate + " Rate.");
        }
        return true;
    }
    ...
    public String getWrapperName() {
        return "Forecast Service Receiver";
    }
